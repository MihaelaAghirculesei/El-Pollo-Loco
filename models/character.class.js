class Character extends MovableObject {
  ...
  IMAGES_IDLE = Array.from(
    { length: 10 },
    (_, i) => `img_pollo_locco/img/2_character_pepe/1_idle/idle/I-${i + 1}.png`
  );
  IMAGES_SLEEPING = Array.from(
    { length: 10 },
    (_, i) =>
      `img_pollo_locco/img/2_character_pepe/1_idle/long_idle/I-${i + 11}.png`
  );
  IMAGES_WALKING = Array.from(
    { length: 6 },
    (_, i) => `img_pollo_locco/img/2_character_pepe/2_walk/W-${21 + i}.png`
  );
  IMAGES_JUMPING = Array.from(
    { length: 9 },
    (_, i) => `img_pollo_locco/img/2_character_pepe/3_jump/J-${31 + i}.png`
  );
  IMAGES_HURT = Array.from(
    { length: 3 },
    (_, i) => `img_pollo_locco/img/2_character_pepe/4_hurt/H-${41 + i}.png`
  );
  IMAGES_DEAD = Array.from(
    { length: 7 },
    (_, i) => `img_pollo_locco/img/2_character_pepe/5_dead/D-${51 + i}.png`
  );
  ...
  handleStateTransitions() {
    setInterval(() => {
      ...
      this.currentState = this.endbossDead()
        ? "active"
        : t > 6000 && !this.isDead()
        ? "sleeping"
        : t > 3000
  ...
  endbossDead() {
    ...
    return eb
      ? eb.isEnemyDead()
      : (console.warn("Endboss not found in enemies array."), false);
  }
}
